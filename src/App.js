...
class App extends Component {
  ...
  decrementCartItemQuantity = id => {
    ...
    const decrementItem = cartList.find(eachItem => eachItem.id === id)
  ...
  addCartItem = product => {
    const {cartList} = this.state
    const findCartItem = cartList.find(eachItem => eachItem.id === product.id)
    if (findCartItem) {
      this.setState(prevState => ({
        cartList: prevState.cartList.map(eachCartItem => {
          if (findCartItem.id === eachCartItem.id) {
